...
const Question: React.FC = () => {
  ...
  useEffect(() => {
    chapter = 1;
    axios.post('http://localhost:4000/api/question/ch1').then((data) => {
  ...
  const next = () => {
    ...
    try {
      ...
      if (chapter === 1.5 || chapter === 2) {
        axios.post('http://localhost:4000/api/question/ch2', { result }).then((data) => {
        ...
        axios.post('http://localhost:4000/api/question/ch3', { result }).then((data) => {
          questionList.current = questionList.current.concat(data.data.data);
          setText(questionList.current[refValue.current].question);
          setAnswer(questionList.current[refValue.current].answer.map((v) => v.text));
        });
        ...
        axios.post('http://localhost:4000/api/question/result', {
          result,
          name,
          gender,
        }).then((res) => {
